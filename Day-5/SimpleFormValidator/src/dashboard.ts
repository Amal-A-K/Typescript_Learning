import { getUsers, getCurrentUser, saveUsers, User, UserRole } from "./script";
...
function initializeUserList(currentUser: User) {
...
function editUser(index: number) {
  ...
  const validRoles: UserRole[] = ["admin", "editor", "viewer", "moderator"];

  const newName = prompt("Enter new name:", user.name);
  const newAge = prompt("Enter new age:", user.age.toString());
  const newRole = prompt(`Enter new role (${validRoles.join("/")}):`, user.role) as UserRole;
